...
public class PlayerManagement extends SQLManagement {
	 ...
	 * 
	 * @param sqlCon
	 *            a valid and open {@link SQLConnection}
	 */
	 ...
	 */
	public void addPlayer(Player p, int teamID) {
		long worth = 2000000;
		String sqlQuery = "INSERT INTO Spieler (Name,Verein_ID, Position, Punkte, Marktwert, Nummer, SportalID, Birthday, PicturePath) " + "VALUES ('" + p.getName() + "','" + teamID + "','"
				+ p.getPosition() + "','" + "0','" + worth + "','" + p.getNumber() + "','" + p.getSportalID() + "','" + p.getBirthdateString() + "','" + p.getAbsolutePictureURL() + "')";
	...
	private void addTeam(RealTeam t, int leagueID) {
		teamCounter++;
		try {
			String sqlQuery = "INSERT INTO Verein (Vereinsname, Liga_ID, LogoPath) Values ('" + t.getTeamName() + "','" + leagueID + "','" + t.getLogoURL() + "')";
			sqlCon.sendQuery(sqlQuery);
	 ...
	 *            {@link Manager};
	 * @param plays
	 *            true = player is in the formation, false = he is not.
	 */
	...
	/**
	 * This function updated the manager formation in the database. It creates
	 * the required query and sends it.
	 * 
	 * @param managerID
	 *            the ID of the {@link Manager}, that should be updated.
	 * @param defenders
	 *            the number of defenders
	 * @param middfielders
	 *            the number of middfielders
	 * @param offensives
	 *            the number of offensives
	 */
	public void setManagersFormation(int managerID, int defenders, int middfielders, int offensives) {
		String sqlQuery = "UPDATE Manager Set Anzahl_Abwehr = " + defenders + ", Anzahl_Mittelfeld=" + middfielders + ", Anzahl_Stuermer=" + offensives + " Where ID = " + managerID;
		sqlCon.sendQuery(sqlQuery);

	}

	/**
	 * This function creates a {@link Player} object by the given {@link ResultSet}. The {@link ResultSet} has to be valid for this!
	 * @param rs a VALID {@link ResultSet}, where all information are set. Otherwise the {@link Player} object can be incomplete. 
	...
	/**
	 * This function iterates over the given {@link ResultSet} and calls the
	 * getPlayerFromResult for each entry.
	 * 
	 * @param rs
	 *            the {@link ResultSet} with the {@link Player} information
	 ...
	 * 
	 * @param manager
	 *            the {@link Manager}, that should be updated.
	 */
	...
	/**
	 * This function clears the "Manschaft Copy" table and copies the
	 * "Mannschaft" table to the "Manschaft Copy" table.
	 */
