...
public class CommunityAuthentificationAck extends CallableAbstract {
	...
	private void handleEnter(JSONObject authentificationACK) {
		if (!authentificationACK.getBoolean("existCommunity")||!authentificationACK.getBoolean("correctPassword")) {
			AlertUtils.createSimpleDialog(AlertUtils.COMMUNITY_ENTER_TITLE, AlertUtils.COMMUNITY_ENTER_WORKED_NOT_HEAD, AlertUtils.COMMUNITY_ENTER_WRONG_AUTHENTIFICATION, AlertType.ERROR);
		}
		else if(!authentificationACK.getBoolean("userDoesNotExist")){
			AlertUtils.createSimpleDialog(AlertUtils.COMMUNITY_ENTER_TITLE, AlertUtils.COMMUNITY_ENTER_WORKED_NOT_HEAD, AlertUtils.COMMUNITY_ENTER_ALREADY_EXIST, AlertType.ERROR);
		}
		else{
			AlertUtils.createSimpleDialog(AlertUtils.COMMUNITY_ENTER_TITLE, AlertUtils.COMMUNITY_ENTER_WORKED_HEAD, AlertUtils.COMMUNITY_ENTER_WORKED_MESSAGE, AlertType.INFORMATION);
